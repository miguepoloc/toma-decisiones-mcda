...
export default function GeoTour({ steps, open, onClose, onTab }: Props) {
  ...
  useLayoutEffect(() => {
    if (!open || !step) return;
    const t = setTimeout(() => { document.querySelector(step.sel)?.scrollIntoView({ block: 'nearest', behavior: 'smooth' }); }, 120);
  ...
  return (
    <div className="gv-tour" role="dialog" aria-label="Recorrido guiado">
      ...
      <div className="gv-tour-card" style={{ left, top, width: W }}>
        ...
        <div className="acts">
          ...
          <button type="button" className="btn primary sm" onClick={() => (last ? close() : setI(i + 1))}>{last ? 'Terminar' : 'Siguiente'}</button>
        </div>
      </div>
    </div>
  );
